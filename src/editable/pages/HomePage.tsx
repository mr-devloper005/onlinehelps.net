...
export default async function HomePage() {
  // The public focus is the Reference Library (pdf). Force the primary feed to
  // pull from pdf when it is enabled so no other content types leak onto home.
  const preferred: TaskKey = 'pdf'
  const primaryTask = ((SITE_CONFIG.tasks.find((task) => task.enabled && task.key === preferred)?.key)
    || SITE_CONFIG.tasks.find((task) => task.enabled)?.key
    || preferred) as TaskKey
  ...
  return (
    <EditableSiteShell>
      <main>
        ...
        <div className="mx-auto w-full max-w-[var(--editable-container)] px-[var(--editable-container-pad)] py-6">
