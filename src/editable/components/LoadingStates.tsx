...
export function PageLoadingState({ label = 'Loading', className }: LoadingStateProps) {
  return (
    <div className={cn('mx-auto w-full max-w-[var(--editable-container)] px-[var(--editable-container-pad)] py-20', className)} aria-live="polite" aria-busy="true">
      <p className="editable-mono text-[11px] uppercase tracking-[0.24em] text-[var(--slot4-accent)]">{label}</p>
      <PulseBlock className="mt-6 h-14 w-3/4 max-w-3xl" />
      <PulseBlock className="mt-4 h-5 w-2/3 max-w-2xl" />
      <div className="mt-14 grid gap-5 md:grid-cols-3">
        {[0, 1, 2].map((item) => (
          <div key={item} className="rounded-[24px] border border-[var(--editable-border)] bg-[var(--slot4-panel-bg)] p-6">
  ...
  return (
    <div className={cn('grid gap-5 sm:grid-cols-2 lg:grid-cols-3', className)} aria-live="polite" aria-busy="true">
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="rounded-[24px] border border-[var(--editable-border)] bg-[var(--slot4-panel-bg)] p-5">
...
export function DetailLoadingState({ label = 'Loading reference', className }: LoadingStateProps) {
  return (
    <div className={cn('mx-auto grid w-full max-w-[var(--editable-container)] gap-10 px-[var(--editable-container-pad)] py-20 lg:grid-cols-[0.8fr_1.2fr]', className)} aria-live="polite" aria-busy="true">
